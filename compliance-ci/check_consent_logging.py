...
def scan_for_consent_and_audit(spec):
    ...
    for p, methods in paths.items():
        # methods can include vendor extensions at the path level and operation objects
        # check path-level vendor extensions
        if isinstance(methods, dict):
            if methods.get('x-consent-scope'):
                consent_hits.append(p + ' (path)')
            if methods.get('x-audit-log'):
                audit_hits.append(p + ' (path)')

        for m, info in (methods or {}).items():
            # some entries in the methods dict are vendor extensions or parameters; skip non-dict ops
            if not isinstance(info, dict):
                continue
            desc = info.get('description') or ''
            if 'consent' in desc.lower() or 'consent' in (info.get('summary') or '').lower():
                consent_hits.append(p + ' ' + m)
            # detect vendor extensions on operation level
            if info.get('x-consent-scope'):
                consent_hits.append(p + ' ' + m + ' (x-consent-scope)')
            if info.get('x-audit-log'):
                audit_hits.append(p + ' ' + m + ' (x-audit-log)')
    ...
    for name, schema in (comps.get('schemas') or {}).items():
        ...
        # detect vendor extensions on schemas (rare but possible)
        if schema.get('x-consent-scope'):
            consent_hits.append('schema.' + name + ' (x-consent-scope)')
        if schema.get('x-audit-log'):
            audit_hits.append('schema.' + name + ' (x-audit-log)')
